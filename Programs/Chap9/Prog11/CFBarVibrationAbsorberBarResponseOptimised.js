...
let lSlider, dSlider, d2Slider, sliders = [];
let lBeam = 1.2, dBeam1 = 0.15, kBeam = 0, dBeam2 = 0.1, aBeam = 0,
    values = [lBeam, dBeam1, dBeam2],
    factors = [100, 100, 100];
let inputs = [], inputAttr = [[0.5, 2, 0.01], [0.05, 0.3, 0.01], [0.05, 0.3, 0.01]];
let graph;
let r1 = [], r2 = [], r3 = [];
...
function draw() {
    ...
    push();
    textAlign(CENTER);
    text(`Optimum values\nAbsorber stiffness: ${Math.round(kBeam)}\nHysteric damping ratio: ${aBeam.toFixed(4)}`, 125, 250);
    pop();
...
function createSliders() {
    ...
    sliders = [lSlider, dSlider, d2Slider];
    let ypos = 60;
...
function optimise() {
    kBeam = 5e5;
    aBeam = 0.4;
    let k10 = kBeam / 10, a10 = aBeam / 10;
    let m1, m2;
    for (let i = 0; i < 6; i++) {
        calcResponse();
        m1 = Math.max(...r3);
        (i % 2 ? aBeam += a10 : kBeam += k10);
        calcResponse();
        m2 = Math.max(...r3);
        let f = 1;
        if (m1 <= m2) {
            f = -1;
            (i % 2 ? aBeam -= 2 * a10 : kBeam -= 2 * k10);
            calcResponse();
            m2 = Math.max(...r3);
        }
        while (m1 > m2) {
            m1 = m2;
            (i % 2 ? aBeam += f * a10 : kBeam += f * k10);
            calcResponse();
            m2 = Math.max(...r3);
        }
        (i % 2 ? a10 *= 0.1 : k10 *= 0.1);
