...
let lBeam = 1.2, dBeam = 0.15, mBeam = 2, aBeam = 0, kBeam = 0, values = [lBeam, dBeam, mBeam],
    factors = [100, 100, 100];
let inputs = [], inputAttr = [[0.5, 2, 0.01], [0.05, 0.3, 0.01], [0.05, 5, 0.01]];
let graph;
...
function draw() {
    ...
    push();
    textAlign(CENTER);
    text(`Optimum Values\nAbsorber Stiffness: ${Math.round(kBeam)} N/m\nHysteric damping ratio: ${aBeam.toFixed(4)}`, 120, 250);
    pop();
...
function findMaxR2() {
    let max = 0;
    for (let i = 1; i < 201; i += 1 / 4) {
        let v = calcResponse2(i);
        if (v > max) max = v;
    }
    return max;
}
...
function optimiseR2() {
    let a = PI * (dBeam ** 2) / 4;
    let inertia = a * (dBeam ** 2) / 16;
    kBeam = mBeam * (3.52 * Math.sqrt(2E11 * inertia / (7800 * a)) / lBeam / lBeam) ** 2;
    aBeam = 0.2;
    let k10 = kBeam * 0.1;
    let a10 = aBeam * 0.1;
    let m1, m2;
