let t = 0;
let m1Slider, k1Slider, c1Slider, x0Slider, m2Slider, k2Slider, c2Slider, fSlider;
let sliders = [];
let playButton, clearButton;
let playing;
let mMass1 = 0.1, mMass2 = 0.1, kMass1 = 20, kMass2 = 20, x0 = 3, fMass = 1, cMass1 = 0.5, cMass2 = 0.01,
    values = [mMass1, kMass1, cMass1, x0, mMass2, kMass2, cMass2, fMass],
    factors = [10, 10, 100, 10, 10, 10, 100, 100];
let inputs = [],
    inputAttr = [[0.1, 2, 0.1], [1, 40, 0.1], [0, 1, 0.01], [-10, 10, 0.1], [0.1, 2, 0.1], [1, 40, 0.1], [0, 1, 0.01], [0, 2, 0.1]];
let plot;
let curData;
let dt;
...
function createSliders() {
    ...
    x0Slider = createSlider(-100, 100, x0 * 10);
    m2Slider = createSlider(1, 20, mMass2 * 10);
    k2Slider = createSlider(10, 400, kMass2 * 10);
    c2Slider = createSlider(0, 100, cMass2 * 100);
    fSlider = createSlider(1, 200, fMass * 100);
    sliders = [m1Slider, k1Slider, c1Slider, x0Slider, m2Slider, k2Slider, c2Slider, fSlider];
    let ypos = 130;
...
function eq(points) {
    let scale = 4200, fIn = [], disp1 = [0], s1 = [0], v1 = [0], f1 = [], disp2 = [0], s2 = [0], v2 = [0],
        f2 = [], dt2 = dt * 2;
    for (let i = 0; i < points; i++) {
        ...
        fIn.push((x0 / 1000) * Math.sin(fMass * TWO_PI * (i * dt2 - 1)));
    }
    for (let i = 0; i < points; i++) {
        f1[i] = (-kMass1 * (s1[i] - fIn[i]) - cMass1 * (v1[i] - (fIn[i + 1] - fIn[i]) / dt2) + kMass2 * (s2[i] - s1[i]) + cMass2 * (v2[i] - v1[i])) / mMass1;
        ...
        for (let j = 1; j < 4; j++) {
            ...
            f1[i + j] = (-kMass1 * (s1[i + j] - fIn[i + j]) - cMass1 * (v1[i + j] - (fIn[i + j + 1] - fIn[i + j]) / dt2) + kMass2 * (s2[i + j] - s1[i + j]) + cMass2 * (v2[i + j] - v1[i + j])) / mMass1;
