...
function draw() {
    ...
    text(`Detuned Frequency: ${(Math.sqrt((k2Slider.value() / 10) / (m2Slider.value() / 10)) / 2 / Math.PI).toFixed(3)} Hz`, 60, 60);
...
function updateSliders() {
    ...
    text(':                 Ns/m', 51, 240);
    drawSubscript(163, 120, "m", '2');
    text(':                 kg', 180, 120);
    drawSubscript(168, 180, 'k', '2');
    text(':                 N/m', 180, 180);
    pop();
}
...
function eq(points) {
    let scale = 400, fIn = [], disp1 = [0], s1 = [0], v1 = [0], f1 = [], disp2 = [0], s2 = [0], v2 = [0],
    ...
    for (let i = 0; i < points; i++) {
        ...
        fIn.push(Math.sin(win * (i * dt2 - 1)));
    }
    for (let i = 0; i < points; i++) {
        f1[i] = (fIn[i] - kMass1 * s1[i] - cMass1 * v1[i] + kMass2 * (s2[i] - s1[i])) / mMass1;
        ...
        for (let j = 1; j < 4; j++) {
            ...
            f1[i + j] = (fIn[i + j] - kMass1 * s1[i + j] - cMass1 * v1[i + j] + kMass2 * (s2[i + j] - s1[i + j])) / mMass1;
